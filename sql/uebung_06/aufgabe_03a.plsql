BEGIN
  ...
  FOR rec IN (
    ...
    FROM countries co
      ...
      INNER JOIN contacts c ON (c.supplier_id = su.supplier_id)
    ...
    DBMS_OUTPUT.PUT_LINE('- ' || rec.country_name || '; ' || rec.state_name || '; ' || rec.city_name || '; ' || rec.city_postalcode || '; ' || rec.supplier_name || '; ' || rec.contact_firstname || '; ' || rec.contact_lastname);
  END LOOP;
  DBMS_OUTPUT.PUT_LINE('+--------------------------------------+');
